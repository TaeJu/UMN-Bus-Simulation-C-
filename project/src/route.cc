...
bool Route::ifContain(std::string id_) {
  for (std::vector<StopData>:: iterator it = stops_data_.begin();
  it != stops_data_.end(); it++) {
    if ((*it).id == id_) {
      stop_data_ = (*it);
      return true;
    }
...
void Route::NotifyStopObserver(StopData* Info) {
      IObservable::NotifyStopObserver(Info);
}
...
void Route::UpdateRouteData() {
    ...
    for (auto* s : stops_) {
        ...
        stops_data_.push_back(stopData);
    }
