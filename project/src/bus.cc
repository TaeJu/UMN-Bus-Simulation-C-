...
void Bus::ClearBusOstr() {
  bus_ostr.str("");
  bus_ostr.clear();
}

void Bus::ClearPasOstr() {
  passenger_ostr.str("");
  passenger_ostr.clear();
}
...
void Bus::NotifyObserver(BusData* Info) {
    IObservable::NotifyObserver(Info);
}
    ...
    if (distance_remaining_ <= 0) {
        ...
        if (outgoing_route_->IsAtEnd()) {
            ...
            if (!incoming_route_->IsAtEnd()) {
                ...
                // before current_route gets set to nextstop
                // check if the bus has any passenger to unload
                // next stop and check if nextstop has no passengers to load

                if (UnloadPassengers() == 0 && GetNextStop()->
                GetNumPassengersPresent() == 0) {
                  std::cout << name_ << " skipping " <<
                  GetNextStop()->GetId() << std::endl;
                }
                ...
                // but, if we didn't handle any
                // passengers here, any negative will
        ...
        if (this->UnloadPassengers() == 0 && this->
        GetNextStop()->GetNumPassengersPresent() == 0) {
          std::cout << name_ << " skipping " <<
           GetNextStop()->GetId() << std::endl;
        }
...
void Bus::UpdateBusData() {
    ...
    std::cout << GetNumPassengers() << std::endl;
